/**
 * Sitecore adapter for the offers family. Unwraps Sitecore field
 * objects into the flat-prop shapes the React renderings consume.
 *
 * Three rendering targets: `OffersListGrid`, `OffersCarousel`, and
 * `OffersSearchExperience`. All three share the same datasource
...
export interface SitecoreOffersFields {
  Title?: TextSource;
  Lead?: TextSource;
  Offers?: SitecoreOfferItem[];
  SearchConfig?: Field<string>;
}

export interface SitecoreOffersParams {
  /** Card-shape choices flow from datasource fields onto child cards. */
  CardVariant?: string;
...
export function adaptOfferItems(
  items: SitecoreOfferItem[] | undefined,
): OfferFlatItem[] {
  ...
  return items.map((item) => {
    ...
    return {
      id: item.id,
      title: fields?.OfferText?.value,
      href: item.url,
      extras: {
        text: fields?.OfferText,
        discountToken: fields?.DiscountToken?.value,
      },
    };
  });
}
...
const ALLOWED_CARD_VARIANTS = ["simple", "complex", "deal"] as const;
const ALLOWED_ACTIONS = ["auto", "link", "copy", "none"] as const;
...
export function adaptListGridProps({
  ...
  fields?: SitecoreOffersFields;
  params?: SitecoreOffersParams;
}) {
  return {
    title: fields?.Title,
    lead: fields?.Lead,
    items: adaptOfferItems(fields?.Offers),
    searchConfig: adaptSearchConfig(fields?.SearchConfig),
    headingLayout: params?.HeadingLayout,
    headingSize: params?.HeadingSize,
    cardVariant: oneOf(params?.CardVariant, ALLOWED_CARD_VARIANTS, "simple"),
    action: oneOf(params?.Action, ALLOWED_ACTIONS, "auto"),
