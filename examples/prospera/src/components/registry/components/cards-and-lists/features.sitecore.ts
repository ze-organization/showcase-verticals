/**
 * Sitecore adapter for the features family. Unwraps Sitecore field
 * objects into the flat-prop shapes the React renderings consume.
 *
 * Three rendering targets: `FeaturesListGrid`, `FeaturesCarousel`, and
 * `FeaturesSearchExperience`. All three share the same datasource
 * template, so a single set of adapters fans out to all of them.
 *
 * The Treelist-driven recipe path is handled directly here with a
 * simple field unwrapper.
 */
...
import type { FeatureExtras, FeatureFlatItem } from "./features-list-grid";

/**
 * Raw shape of a feature Sitecore item under `fields.Features`. Treelist
 * items resolve to this when curated; placeholder mode bypasses this
 * entirely (children render as Sitecore placeholders).
 */
export interface SitecoreFeatureItem {
  id: string;
  displayName?: string;
  name?: string;
  url?: string;
  fields?: {
    ...
    /**
     * Named vector icon (`icon-name@1`) — the one icon authoring
     * surface since the 2026-07 icon normalization.
     */
    IconName?: Field<string>;
    Image?: Field<{ src?: string; alt?: string }>;
    Link?: Field<{ href?: string; text?: string }>;
  };
}

export interface SitecoreFeaturesFields {
  /** Optional kicker line above the title (small-caps eyebrow). */
  Eyebrow?: TextSource;
  Title?: TextSource;
  Lead?: TextSource;
  Features?: SitecoreFeatureItem[];
  SearchConfig?: Field<string>;
}

export interface SitecoreFeaturesParams {
...
/**
 * Treelist-shaped Sitecore items → `FeatureFlatItem[]`. The MediaBanded
 * / MediaStacked / IconTile card variants want richer extras than the
 * `FlatItem` core, so we ferry the raw field jsonValues through
 * `extras` for downstream `toViewItem` to consume verbatim.
 */
export function adaptFeatureItems(
  items: SitecoreFeatureItem[] | undefined,
): FeatureFlatItem[] {
  ...
  return items.map((item) => {
    ...
    const imageField = fields?.Image as
      ...
      | undefined;
    const linkField = fields?.Link as
      | { value?: { href?: string; text?: string } }
      | undefined;
    const image = imageField?.value;
    const extras: FeatureExtras = {
      titleField,
      descriptionField,
      imageField,
      // Named glyph (icon-name@1) — IconTile's icon surface; wins over
      // the Image when it resolves.
      iconName: item.fields?.IconName?.value,
      linkField,
    };
    return {
      id: item.id,
      ...
      href: linkField?.value?.href ?? item.url,
      extras,
    };
  });
}
...
export function adaptListGridProps({
  ...
  fields?: SitecoreFeaturesFields;
  params?: SitecoreFeaturesParams;
}) {
  return {
    title: fields?.Title,
    lead: fields?.Lead,
    eyebrow: fields?.Eyebrow,
    items: adaptFeatureItems(fields?.Features),
