/**
 * Sitecore adapter for the `persons` cards-and-lists family. Unwraps
 * the recipe-shaped fields into the flat `PersonFlatItem` shape the
 * React renderings (`person-list-grid`, `person-carousel`) consume.
 * Maps a curated Treelist of `person-card@1` items into the common
 * `extras` envelope alongside the top-level FlatItem fields.
 *
 * A "doctor" is just a flavor of person — the `Role` field carries
 * the specialty/title. This adapter has no doctor-specific logic.
 */
...
import type { PersonFlatItem } from "./person-list-grid";

export interface SitecorePersonCardFields {
  /** Full name. */
  FullName?: Field<string>;
  /** Job title / position / specialty. */
  Role?: Field<string>;
  /** Long-form bio narrative (rich-text). */
  Bio?: { value?: string };
...
export interface SitecorePersonItem {
  id: string;
  displayName?: string;
  name?: string;
  url?: string;
  fields?: SitecorePersonCardFields;
}

export interface SitecorePersonsFields {
  Title?: TextSource;
  Lead?: TextSource;
  Persons?: SitecorePersonItem[];
  SearchConfig?: Field<string>;
}

export interface SitecorePersonsParams {
  /** Card-shape choices. */
  CardVariant?: string;
...
export function adaptPersonItems(
  items: SitecorePersonItem[] | undefined,
): PersonFlatItem[] {
  ...
  return items.map((item) => {
    ...
    return {
      id: item.id,
      title: fields.FullName?.value,
      description: (fields.Bio as { value?: string } | undefined)?.value,
      image,
      href,
      extras: {
        role: fields.Role,
        bio: fields.Bio as RichTextSource | undefined,
        eyebrow: fields.Eyebrow,
        email: fields.Email,
        phone: fields.Phone,
        image: fields.Image as ImageSource | undefined,
...
export function adaptListGridProps({
  ...
  fields?: SitecorePersonsFields;
  params?: SitecorePersonsParams;
}) {
  return {
    title: fields?.Title,
    lead: fields?.Lead,
    items: adaptPersonItems(fields?.Persons),
