...
/**
 * Recipe for `PricingCarousel` — carousel layout rendering for the
 * pricing family. Same datasource shape as `pricing-list-grid@1`, so
 * the two are marked compatible (authors swap layout via the
 * rendering picker without re-binding their plans).
 *
 * References `card-carousel-params@1` for autoplay, slides per view,
 * navigation, pagination, and the shared base/styling params.
 */
export const pricingCarouselRecipe = {
  kind: "component-template",
  schemaVersion: "1",
  handle: "pricing-carousel@1",
  icon: componentIcons["pricing-carousel@1"],
  name: "pricing-carousel",
  displayName: "Pricing Carousel",
  description:
    "Pricing carousel. Same datasource as pricing-list-grid (composed / curated / search). Variants: Default, FullBleed, FeatureSpotlight.",
  ...
  fields: [
    ...
    },
    {
      ...
      sitecore: {
        type: "treelist",
        hint: "Curated mode — pick pricing-card items. Leave empty to use the placeholder for composed mode, or populate SearchConfig for search mode.",
...
export default pricingCarouselRecipe;
