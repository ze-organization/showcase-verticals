...
export default defineConfig({
  ...
  // Editing hosts render on demand; skip SSG until a real site tree exists.
  generateStaticPaths: false,
  redirects: {
    enabled: true,
    // Keep in lockstep with `src/i18n/routing.ts` locales.
    locales: ['en', 'fr-FR', 'de-DE', 'es-ES', 'ar-SA'],
  },
  multisite: {
    enabled: true,
